...
```python
# Pydantic 模型：用 Python 类型注解定义数据结构，自动生成 JSON Schema
# 比手写 JSON Schema 更简洁、更不易出错，且支持 IDE 类型检查
from pydantic import BaseModel

class Product(BaseModel):
    product: str                    # 产品名称（必填，字符串类型）
    price: float                    # 价格（必填，浮点数类型）
    in_stock: bool                  # 是否有库存（必填，布尔类型）
    categories: list[str] = []      # 分类列表（可选，默认空列表）
```
...
```python
# JSON Schema 验证器：从零实现，检查 Python 对象是否匹配给定的 JSON Schema
# 核心思路：递归遍历 schema 树，逐层验证数据的类型、必填字段、约束条件
import json

def validate_schema(data, schema):
    """验证入口：收集所有错误后一次性返回"""
    errors = []
    _validate(data, schema, "", errors)  # path="" 表示根路径
    return errors

def _validate(data, schema, path, errors):
    """递归验证函数：根据 schema 类型分派到不同的验证逻辑"""
    schema_type = schema.get("type")

    # 验证对象类型：检查是否为 dict，必填字段是否存在，递归验证每个属性
    if schema_type == "object":
        if not isinstance(data, dict):
            errors.append(f"{path}: expected object, got {type(data).__name__}")
            return
        # 检查所有 required 字段是否都存在
        for key in schema.get("required", []):
            if key not in data:
                errors.append(f"{path}.{key}: required field missing")
        # 递归验证每个属性值是否匹配其子 schema
        properties = schema.get("properties", {})
        for key, value in data.items():
            if key in properties:
                _validate(value, properties[key], f"{path}.{key}", errors)

    # 验证数组类型：检查是否为 list，长度约束，递归验证每个元素
    ...
    # 验证布尔类型：注意 Python 中 bool 是 int 的子类，需单独检查
    elif schema_type == "boolean":
        if not isinstance(data, bool):
            errors.append(f"{path}: expected boolean, got {type(data).__name__}")

    # 验证整数类型：bool 在 Python 中是 int 子类，所以要排除 bool
...
```python
# SchemaField：简化版的 Pydantic 字段描述符，手动定义字段类型和约束
# 实际项目中直接用 Pydantic BaseModel 即可，这里是为了展示原理
class SchemaField:
    def __init__(self, field_type, required=True, default=None, enum=None, minimum=None, maximum=None):
        self.field_type = field_type   # Python 类型：str, int, float, bool, list
        self.required = required       # 是否为必填字段
        self.default = default         # 默认值（用于可选字段）
        self.enum = enum               # 枚举约束：只允许特定值
        self.minimum = minimum         # 数值最小值约束
        self.maximum = maximum         # 数值最大值约束

# 将 Python 类型映射为 JSON Schema 类型名
def python_type_to_schema(field):
    ...
    # list 类型映射为 array，元素默认为 string
    elif field.field_type == list:
        schema["type"] = "array"
        schema["items"] = {"type": "string"}
    # dict 类型视为自定义 schema，直接使用
...
# 将一组 SchemaField 转换为完整的 JSON Schema 对象
# 功能等价于 Pydantic 的 model_json_schema()，但手动实现
...
```python
# 约束解码模拟器：展示在 JSON 生成过程中，每个位置允许哪些 token 类型
# 核心思想：根据已生成的部分 JSON 的最后一个字符，推断下一步合法的 token
# 这就是 OpenAI 结构化输出、Outlines、XGrammar 在底层做的事情的简化版
def next_valid_tokens(partial_json, schema):
    stripped = partial_json.strip()

    # 空输入：只能以 '{' 开始一个 JSON 对象
    if not stripped:
        return ["{"]

    # 如果已经能完整解析，说明 JSON 已完成，输出结束标记
    try:
        json.loads(stripped)
        return ["<EOS>"]
    ...
    if last_char == "{":
        ...
        if stripped.endswith('":'):
            # 键名后面跟冒号：值必须开始（字符串/数字/布尔/null/数组/对象）
        ...
        # 数字后面：继续数字、小数点、或结束当前值
        return ["0-9", ".", ",", "}", "]"]
    elif last_char == "}":
        # 对象闭合：后面可以是逗号（继续）、闭合括号、或结束
        return [",", "}", "]", "<EOS>"]
    elif last_char == "]":
        # 数组闭合：类似对象闭合
        return [",", "}", "<EOS>"]
    elif last_char == "[":
        # 数组开始：可以是任何值类型，或空数组闭合
        return ['"', "0-9", "true", "false", "null", "{", "[", "]"]
    else:
        return ["any"]

# 演示约束解码：逐步构建 JSON，展示每一步允许的 token
def demonstrate_constrained_decoding():
    ...
    partial_states = [
        ...
        '{"product": "Sony"',                          # 字符串值
        '{"product": "Sony",',                         # 下一个键值对
        '{"product": "Sony", "price":',               # 第二个键的值
        '{"product": "Sony", "price": 348',           # 数字值
        '{"product": "Sony", "price": 348}',          # 对象闭合
    ]
...
```python
# 模拟 LLM 提取：根据输入文本返回 JSON 字符串
# 第一次尝试可能返回不完全匹配 schema 的结果（如多余字段），
# 用于演示验证+重试机制的必要性
def simulate_llm_extraction(text, schema, attempt=0):
    if "headphones" in text.lower() or "sony" in text.lower():
        if attempt == 0:
            # 第一次：返回包含 categories 的完整结果（schema 中 categories 是可选的，这没问题）
...
# 带重试的提取流水线：调用 LLM → 解析 JSON → 验证 Schema → 失败则重试
# 这是生产环境中处理 LLM 结构化输出的标准模式
def extract_with_retry(text, schema, max_retries=3):
    for attempt in range(max_retries):
        # 第1步：调用（模拟的）LLM 获取原始 JSON 字符串
        ...
        # 第3步：验证数据是否匹配 schema（语义层面）
        errors = validate_schema(data, schema)
        if not errors:
            return data  # 验证通过，返回结果

        print(f"  Attempt {attempt + 1}: Schema validation errors -- {errors}")

    return None  # 所有重试都失败

# 产品信息提取的 JSON Schema 定义
# required 字段：product、price、in_stock（缺一不可）
# 可选字段：categories（数组，元素为字符串）
product_schema = {
    "type": "object",
    "properties": {
        "product": {"type": "string"},
        "price": {"type": "number", "minimum": 0},       # 价格不能为负数
        "in_stock": {"type": "boolean"},
...
```python
# 完整演示：串联所有组件，展示从 schema 定义到提取验证的完整流水线
def run_demo():
    print("=" * 60)
    print("  Structured Output Pipeline Demo")
    print("=" * 60)

    # 第1部分：用 SchemaField 定义模型，自动生成 JSON Schema
    print("\n--- Schema Definition ---")
    product_fields = {
        "product": SchemaField(str),
        "price": SchemaField(float, minimum=0),        # 最小值约束：价格≥0
        "in_stock": SchemaField(bool),
        "categories": SchemaField(list, required=False), # 可选字段
    }
    generated_schema = model_to_schema("Product", product_fields)
    print(json.dumps(generated_schema, indent=2))

    # 第2部分：测试验证器——正确数据、错误数据、缺失字段、类型错误
    print("\n--- Schema Validation ---")
    test_cases = [
        ({"product": "Test", "price": 10.0, "in_stock": True}, "Valid object"),
        ({"product": "Test", "price": -5.0, "in_stock": True}, "Negative price"),     # 价格为负，应失败
        ({"product": "Test", "in_stock": True}, "Missing price"),                      # 缺少必填字段
        ({"product": "Test", "price": "ten", "in_stock": True}, "String as price"),    # 类型错误
        ("not an object", "String instead of object"),                                 # 根类型错误
    ]
    ...
    # 第3部分：展示约束解码——每个 JSON 生成位置允许哪些 token
    print("\n--- Constrained Decoding Simulation ---")
    demonstrate_constrained_decoding()

    # 第4部分：端到端提取——从非结构化文本中提取结构化产品信息
    print("\n--- Extraction Pipeline ---")
    texts = [
        "The Sony WH-1000XM5 headphones are priced at $348 and currently available.",
        "The new MacBook Pro 16-inch laptop costs $2499 but is sold out.",
        "This is a random sentence with no product info.",  # 无产品信息的句子
    ]
